...
class OpenAlexGraphDataset:
    def __init__(self, json_path="data/openalex_cs_papers.json"):
        print("Loading sentence model...")
        self.sentence_model = SentenceTransformer('all-MiniLM-L6-v2')
        print("Done!")

        G, max_citations = self._build_networkx_graph(json_path)
        node_features, node_to_idx = self._process_node_features(G, max_citations)
    ...
    def _build_networkx_graph(self, json_path):
        G = nx.Graph()
        max_citations = -1
        ...
        return G, max_citations

    def _process_node_features(self, G, max_citations):
